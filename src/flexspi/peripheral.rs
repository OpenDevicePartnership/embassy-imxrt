...
impl<'a> FlexSpi<'a> {
    ...
    pub unsafe fn configure_command_sequence(&mut self, sequence: CommandSequence) -> Result<(), InvalidCommandSequence> {
    ...
    pub unsafe fn trigger_command_and_wait(&mut self) -> Result<pac::flexspi::intr::R, WaitCommandError> {
        let interrupts = critical_section::with(|_| {
            unsafe { self._trigger_command_and_wait() }
        });
    ...
    ///
    /// You must also ensure that the .data section is executable before calling this function.
    pub unsafe fn trigger_command_and_wait_write(&mut self) -> Result<pac::flexspi::intr::R, super::nor_flash::WriteError> {
        let (stage, interrupts) = critical_section::with(|_| {
            unsafe { self._trigger_command_and_wait_write() }
        });
        self.check_and_clear_command_interrupts(interrupts)
            .map_err(|e| {
                if stage == 0 {
    ...
    unsafe fn _trigger_command_and_wait_write(&mut self) -> (u32, pac::flexspi::intr::R) {
        ...
        }
        {
            ...
            // SAFETY: pac::flexspi::intr::R is a transparent wrapper around a u32.
            unsafe {
                (stage, core::mem::transmute(interrupts))
            }
        }
    }
    ...
    pub fn set_tx_fifo_watermark_u64_words(&mut self, num_u64: u8) {
            ...
            flex_spi.iptxfcr().modify(|_, w|
                unsafe { w.txwmrk().bits(value) }
            );
    }
    ...
    pub fn set_rx_fifo_watermark_u64_words(&mut self, num_u64: u8) {
        let flex_spi = unsafe { pac::Flexspi::steal() };
        let value = num_u64.saturating_sub(1);
        flex_spi.iprxfcr().modify(|_, w| unsafe {
            w.rxwmrk().bits(value)
        });
    }
    ...
    fn check_and_clear_command_interrupts(&mut self, interrupts: pac::flexspi::intr::R) -> Result<pac::flexspi::intr::R, WaitCommandError> {
        let flex_spi = unsafe { pac::Flexspi::steal() };
