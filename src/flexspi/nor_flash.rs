...
impl<'a> FlexSpiNorFlash<'a> {
    ...
    pub unsafe fn new_unchecked(flex_spi: Peri<'a, FLEXSPI>, alignment: FlashAlignment) -> Self {
        ...
        me.read(0x400, &mut fcb_header).unwrap_or_else(|e| panic!("reading FCB header {}", e));
        ...
        for i in 0..16 {
            ...
            // TODO: Report error instead of panicking.
            me.read(0x400 + 0x80 + i * 16, &mut sequence).unwrap_or_else(|e| panic!("failed to read LUT entry {} from FCB: {}", i, e));
    ...
    pub fn read(&mut self, address: u32, buffer: &mut [u8]) -> Result<(), ReadError> {
        ...
        for (i, buffer) in buffer.chunks_mut(128).enumerate() {
            ...
            unsafe {
                ...
                self.flex_spi.trigger_command_and_wait().map_err(ReadError::WaitFinish)?;
            }
    ...
    pub unsafe fn erase_sector(&mut self, address: u32) -> Result<(), WriteError> {
        ...
        unsafe {
            self.flex_spi
                ...
                self.flex_spi.trigger_command_and_wait_write().map_err(WriteError::WaitFinish)?;
        }
