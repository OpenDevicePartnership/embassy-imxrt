...
use super::{
    ...
    REMEDIATON_SLAVE_NAK, TEN_BIT_PREFIX,
};
...
impl<'a> BlockingI2cSlaveWrite<'a> {
    ...
    pub fn handle_part(self, buffer: &mut [u8]) -> Result<WriteResult<BlockingI2cSlaveWrite<'a>>> {
...
impl Drop for BlockingI2cSlaveWrite<'_> {
    fn drop(&mut self) {
        let i2c = self.info.regs;
        let stat = i2c.stat().read();
        if stat.slvpending().is_pending() && (stat.slvstate().is_slave_receive() || stat.slvstate().is_slave_address())
        {
            i2c.slvctl().modify(|_, w| w.slvnack().nack());
        }
    }
